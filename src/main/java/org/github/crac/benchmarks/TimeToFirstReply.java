...
import java.util.stream.Collectors;
    ...
    static {
        String filename = System.getProperty("benchmark.uri.file");
        if (filename == null) {
            targets = Collections.singletonList(URI.create(System.getProperty("benchmark.uri", "http://localhost:8080")));
        } else {
            try {
                targets = Files.readAllLines(Path.of(filename)).stream().map(URI::create).collect(Collectors.toList());
            } catch (IOException e) {
        ...
        for (int i = 0; i < WARMUP + MEASURED; ++i) {
            ...
            try (HttpClient client = HttpClient.newBuilder().version(HttpClient.Version.HTTP_1_1).build()) {
                ...
                while (process.isAlive() && !received.get()) {
                    ...
                    AtomicInteger responseCounter = new AtomicInteger(targets.size());
                    for (URI uri: targets) {
                    ...
                    future.exceptionally(ex -> null).join();
                }
                if (!received.get()) {
